...
from obsidian_agent.tools import list_notes_in_vault, read_note_from_vault, search_notes_in_vault


def test_search_notes_matches_filename_and_body(tmp_path):
    ...
    filename_results = search_notes_in_vault(vault, "AI", max_results=5)
    body_results = search_notes_in_vault(vault, "calling", max_results=5)

    assert filename_results == [{"file_path": "AI Ideas.md", "snippet": "AI Ideas.md"}]
...
def test_empty_search_returns_stable_structure(tmp_path):
    ...
    assert search_notes_in_vault(vault, "missing", max_results=5) == []
