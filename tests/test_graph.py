...
def test_obsidian_question_triggers_search_and_read(tmp_path):
    ...
    assert [call["name"] for call in result["tool_calls"]] == ["retrieve_notes"]
    assert result["notes_content"][0]["file_path"] == "ai.md"
    assert result["notes_content"][0]["chunk_id"] == "ai.md#0"
...
def test_search_failure_retries_without_infinite_loop(tmp_path):
    ...
    search_calls = [call for call in result["tool_calls"] if call["name"] == "retrieve_notes"]
